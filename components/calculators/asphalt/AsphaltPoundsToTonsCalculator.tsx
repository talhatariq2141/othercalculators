...
export default function AsphaltPoundsToTonsCalculator() {
    const [pounds, setPounds] = useState(DEFAULTS.pounds);
    ...
    const result = useMemo(() => {
        const lb = clampNumber(pounds, 0, 100_000_000);
        const tons = lb / 2000;
        const metricTonnes = lb / 2204.62;
    ...
    const handleCalculate = () => {
        if (!isFiniteNumber(pounds) || pounds <= 0) return;
    ...
    const isValid = isFiniteNumber(pounds) && pounds > 0;

    return (
        <section aria-label="Asphalt Pounds to Tons Calculator">
                ...
                <div className="grid gap-6">
                    <fieldset className="grid grid-cols-1">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Weight (Pounds)</label>
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <span className={unitBadgeClasses}>lbs</span>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    >
                        <div ref={resultsInnerRef} className="pt-2">
                            <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
                                <span className="text-[13px] font-medium text-indigo-900/60 uppercase tracking-wider block mb-1">Commercial Weight</span>
                                <div className="flex items-baseline gap-2 mb-6">
                                    <span className="text-[42px] font-bold text-slate-900 tracking-tight">
                                        {formatNumber(result.tons, 4)}
                                    </span>
                                ...
                                <dl className="grid gap-3 sm:grid-cols-1">
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        ...
                                        <dd className="text-[16px] font-semibold text-slate-900">
                                            {formatNumber(result.metricTonnes, 4)} t
                                        </dd>
                                    </div>
                                </dl>
                            </div>
