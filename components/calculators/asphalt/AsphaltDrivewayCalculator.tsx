...
export default function AsphaltDrivewayCalculator() {
    ...
    const result = useMemo(() => {
        const L = clampNumber(length, 0, 1_000_000);
        const W = clampNumber(width, 0, 1_000_000);
    ...
    return (
        <section aria-label="Asphalt Driveway Calculator">
                ...
                <div className="grid gap-6">
                    <fieldset className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Driveway Length (ft)</label>
                            <input
                                type="number"
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Driveway Width (ft)</label>
                            <input
                                type="number"
                                className={inputClasses}
                                value={width}
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Pavement Thickness (in)</label>
                    ...
                    >
                        <div ref={resultsInnerRef} className="pt-2">
                            <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
                                <span className="text-[13px] font-medium text-indigo-900/60 uppercase tracking-wider block mb-1">Recommended Order</span>
                                ...
                                <dl className="grid gap-3 sm:grid-cols-2">
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        <dt className="text-[12px] text-slate-500">Exact Weight (Net)</dt>
                                        <dd className="text-[16px] font-semibold text-slate-900">
                                            {formatNumber(result.tonnage, 2)} tons
                                        </dd>
                                    </div>
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        <dt className="text-[12px] text-slate-500">Total Area</dt>
                                        <dd className="text-[16px] font-semibold text-slate-900">
                                    ...
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        <dt className="text-[12px] text-slate-500">Waste Factor</dt>
                                        <dd className="text-[16px] font-semibold text-slate-900">
                                            {wastePct}% applied
                                        </dd>
                                    </div>
