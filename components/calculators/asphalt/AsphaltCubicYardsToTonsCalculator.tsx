...
export default function AsphaltCubicYardsToTonsCalculator() {
    const [cuYds, setCuYds] = useState(DEFAULTS.cuYds);
    ...
    const result = useMemo(() => {
        const CY = clampNumber(cuYds, 0, 1_000_000);
    ...
    const handleCalculate = () => {
        if (!isFiniteNumber(cuYds) || cuYds <= 0) return;
    ...
    const isValid = isFiniteNumber(cuYds) && cuYds > 0 && isFiniteNumber(density) && density > 0;

    return (
        <section aria-label="Asphalt Cubic Yards to Tons Calculator">
                ...
                <div className="grid gap-6">
                    <fieldset className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Volume (Cubic Yards)</label>
                            <input
                                type="number"
                                className={inputClasses}
                                value={cuYds}
                                onChange={(e) => setCuYds(parseFloat(e.target.value))}
                            />
                        </div>
                        <div className="flex flex-col gap-1.5">
                    ...
                    >
                        <div ref={resultsInnerRef} className="pt-2">
                            <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
                                <span className="text-[13px] font-medium text-indigo-900/60 uppercase tracking-wider block mb-1">Weight Result</span>
                                <div className="flex items-baseline gap-2 mb-6">
                                    <span className="text-[42px] font-bold text-slate-900 tracking-tight">
                                        {formatNumber(result.tons, 2)}
                                    </span>
