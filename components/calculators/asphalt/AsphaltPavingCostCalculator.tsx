...
export default function AsphaltPavingCostCalculator() {
    const [area, setArea] = useState<number>(1000);
    const [materialCost, setMaterialCost] = useState<number>(3);
    ...
    const result = useMemo(() => {
        ...
        const totalCost = area * totalUnitCost;
        return { totalUnitCost, totalCost };
    }, [area, materialCost, laborCost]);
    ...
    const handleCalculate = () => {
        if (area <= 0 || (materialCost + laborCost) <= 0) return;
    ...
    const isValid = area > 0 && (materialCost >= 0) && (laborCost >= 0) && (materialCost + laborCost > 0);

    return (
        <section aria-label="Asphalt Paving Cost Calculator">
            <div className={containerClasses}>
                <div className="mb-6">
                    <h2 className="text-[18px] font-semibold tracking-[-0.01em] text-slate-900">Project Inputs</h2>
                </div>
                ...
                <div className="grid gap-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                            <label htmlFor="materialCost" className="text-[13px] font-medium text-slate-900/80">
                                Material Cost ($/sq ft)
                            </label>
                            <div className="flex items-center gap-2">
                                <input
                            ...
                            <p className="text-[10px] text-slate-500">Typical: $1 – $5 per sq ft.</p>
                        </div>

                        <div className="flex flex-col gap-1.5">
                            <label htmlFor="laborCost" className="text-[13px] font-medium text-slate-900/80">
                                Labor & Prep ($/sq ft)
                            </label>
                            <div className="flex items-center gap-2">
                                <input
                            ...
                            <p className="text-[10px] text-slate-500">Typical: $3 – $10 per sq ft.</p>
                        </div>
                    </div>
                    ...
                    >
                        <div ref={resultsInnerRef} className="pt-2">
                            <div className="rounded-2xl border border-stone-100 bg-white p-6 shadow-sm">
                                <div className="mb-4">
                                    <h3 className="text-[16px] font-semibold text-slate-900">Paving Cost Summary</h3>
                                </div>
                                ...
                                <div className="grid gap-3 sm:grid-cols-2">
                                    <div className="rounded-xl bg-orange-50/50 p-3 border border-orange-100">
                                        <dt className="text-[11px] text-orange-600 uppercase tracking-wider font-bold">Total Area</dt>
                                    ...
                                    <div className="rounded-xl bg-indigo-50/50 p-3 border border-indigo-100">
                                        <dt className="text-[11px] text-indigo-600 uppercase tracking-wider font-bold">Combined Rate</dt>
                                        <dd className="text-[18px] font-semibold text-slate-900 mt-1">
                                            ${formatNumber(result.totalUnitCost, 2)} / sq ft
                                        </dd>
                                    </div>
                                </div>
                                <div className="mt-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                                    <h4 className="text-[13px] font-semibold text-slate-800 mb-2 underline decoration-indigo-500/30">Cost Breakdown</h4>
                                    <div className="space-y-1 text-[13px] text-slate-600">
                                        <div className="flex justify-between">
                                            <span>Materials:</span>
                                            <span className="font-medium">${formatNumber(area * materialCost, 2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Labor & Prep:</span>
                                            <span className="font-medium">${formatNumber(area * laborCost, 2)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
