...
export default function AsphaltSquareFeetToTonsCalculator() {
    ...
    const result = useMemo(() => {
        ...
        const Th = clampNumber(thickness, 0, 100);
        const D = clampNumber(density, 1, 500);

        const volCuFt = A * (Th / 12);
        const lbs = volCuFt * D;
        const tons = lbs / 2000;
    ...
    return (
        <section aria-label="Asphalt Square Feet to Tons Calculator">
                ...
                <div className="grid gap-6">
                    <fieldset className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-medium text-slate-900/80">Surface Area (sq ft)</label>
                            <input
                                type="number"
                            ...
                            <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
                                ...
                                <dl className="grid gap-3 sm:grid-cols-2">
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        <dt className="text-[12px] text-slate-500">Total Weight (lbs)</dt>
                                        <dd className="text-[16px] font-semibold text-slate-900">
                                            {formatNumber(result.lbs, 0)} lbs
                                        </dd>
                                    </div>
                                    <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                        <dt className="text-[12px] text-slate-500">Volume (cu ft)</dt>
                                        <dd className="text-[16px] font-semibold text-slate-900">
