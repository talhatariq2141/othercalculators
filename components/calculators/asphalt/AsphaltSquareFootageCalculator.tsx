...
function formatNumber(n: number, decimals = 2) {
    ...
    // Remove trailing zeros if it's an integer-ish number, but keep 1 decimal if needed
    return s.replace(/\.?0+$/, "");
}
...
export default function AsphaltSquareFootageCalculator() {
    // State
    ...
    const result = useMemo(() => {
        ...
        // Convert to sq yards just in case user needs it
    ...
    const handleAddArea = () => {
        ...
        setHasCalculated(false); // Reset to encourage re-calculation or at least acknowledge change
    };
    ...
    // Theme Classes (Green Theme per Spec)
    ...
    const iconButtonClasses = "rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600 transition disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400";

    const isValid = result.totalSqFt > 0;

    return (
        <section aria-label="Asphalt Square Footage Calculator">
            <div className={containerClasses}>
                <div className="mb-6 flex items-center justify-between">
                    <h2 className="text-[18px] font-semibold tracking-[-0.01em] text-slate-900">Project Areas</h2>
                    <span className="text-[12px] font-medium text-slate-500 bg-white/50 px-2 py-1 rounded-md">
                ...
                <div className="grid gap-4">
                    {areas.map((area, index) => (
                        <fieldset key={area.id} className="relative grid grid-cols-1 gap-4 sm:grid-cols-2 rounded-xl bg-white/60 p-3 border border-green-700/10">
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label htmlFor={`length-${area.id}`} className="text-[13px] font-medium text-slate-900/80">
                                    Length (ft) <span className="text-xs text-slate-400 font-normal">- Area {index + 1}</span>
                                </label>
                                <input
                                    id={`length-${area.id}`}
                                    type="number"
                                    ...
                                    value={area.length || ""}
                                    onChange={(e) => handleUpdateArea(area.id, 'length', parseFloat(e.target.value))}
                                    placeholder="0"
                                />
                            </div>

                            <div className="flex flex-col gap-1.5">
                                <label htmlFor={`width-${area.id}`} className="text-[13px] font-medium text-slate-900/80">
                                    Width (ft)
                                </label>
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        className={inputClasses}
                                        value={area.width || ""}
                                        onChange={(e) => handleUpdateArea(area.id, 'width', parseFloat(e.target.value))}
                                        placeholder="0"
                                    />
                                    {areas.length > 1 && (
                                        <button
                    ...
                    <button
                        ...
                        className="flex items-center justify-center gap-1.5 rounded-xl border border-dashed border-slate-300 bg-white/40 px-4 py-2 text-[13px] font-semibold text-slate-600 hover:border-indigo-400 hover:bg-white hover:text-indigo-600 transition"
                    >
                    ...
                    >
                        <div ref={resultsInnerRef} className="pt-2">
                            <div className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
                                <div className="mb-4">
                                    <h3 className="text-[16px] font-semibold text-slate-900">Total Project Area</h3>
                                </div>

                                <div className="flex items-baseline gap-2 mb-6">
                                    <span className="text-[42px] font-bold text-slate-900 tracking-tight">
                                        {formatNumber(result.totalSqFt, 2)}
                                    </span>
                                    <span className="text-[20px] font-semibold text-slate-600">sq ft</span>
                                </div>

                                <div className="rounded-xl bg-slate-50 p-3 border border-slate-200">
                                    <dt className="text-[12px] text-slate-500 uppercase tracking-wider font-semibold">Square Yards</dt>
                                    <dd className="text-[18px] font-semibold text-slate-900 mt-1">
                                        {formatNumber(result.totalSqYd, 2)} sq yd
                                    </dd>
                                </div>
                                <p className="mt-4 text-[13px] text-slate-500 leading-relaxed">
                                    This is the total surface area of all the shapes you entered. Use this number to estimate sealcoating coverage or paving materials.
                                </p>
                            </div>
                        </div>
                    </div>
